...
- [lustre/server_component] Server components can now always decode `event.detail` from events emit by child Lustre apps, even when it is not explicitly listed in `server_component.include`.
